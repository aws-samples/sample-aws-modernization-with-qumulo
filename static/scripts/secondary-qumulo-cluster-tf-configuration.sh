...
# Qumulo Workshop - One Node Cluster Deployment Script
...
# Function to configure root provider.tf
...
configure_persistent_storage_tfvars() {
    ...
    sed -i "s/^deployment_name[[:space:]]*=.*/deployment_name = \"qum-wks-cls-sec-str\"/" "$tfvars_file"
    ...
    sed -i "s/^soft_capacity_limit[[:space:]]*=.*/soft_capacity_limit = 100/" "$tfvars_file"
...
configure_root_tfvars() {
    ...
    sed -i "s/^deployment_name[[:space:]]*=.*/deployment_name = \"qum-wks-cls-sec\"/" "$tfvars_file"
    ...
    sed -i "s/^private_subnet_id[[:space:]]*=.*/private_subnet_id = \"$PRIVATE_SUBNET_A\"/" "$tfvars_file"
    ...
    sed -i "s/^q_cluster_name[[:space:]]*=.*/q_cluster_name = \"demosec\"/" "$tfvars_file"
    ...
    sed -i "s/^q_node_count[[:space:]]*=.*/q_node_count = 1/" "$tfvars_file"
    sed -i "s/^dev_environment[[:space:]]*=.*/dev_environment = true/" "$tfvars_file"

    # Set up Qumulo DNS Resolver
    sed -i "s/^q_cluster_fqdn[[:space:]]*=.*/q_cluster_fqdn = \"demosec.qumulo.local\"/" "$tfvars_file"
...
display_summary() {
    ...
    echo "  3. Manually run the deployment with: ./deploy-qumulo-cluster.sh $DEPLOYMENT_DIR \"Secondary Workshop Qumulo One Node Instance\""
